...
class mc_detect_ctrl:
    ...
    def __init__(self):
        ...
        ## final
        self.previous_coordinate = previous_coordinate={'robot': np.array((0.23,0.40)), 'ball': np.array((1.45,0.85)), 'enemy_robot': np.array((2.75,1.55))}
        self.coordinate = previous_coordinate={'robot': np.array((0.23,0.40)), 'ball': np.array((1.45,0.85)), 'enemy_robot': np.array((2.75,1.55))}
    ...
    def reach_ball(self):
        while (not self.is_right() and self.found):
            ...
            if theta < 0:
                ...
                self.turn_flag = False
            if self.location['ball']['dist']>170:
                move_cmd = Twist()
                move_cmd.linear.x = 1
                move_cmd.angular.z = 0.1 if self.turn_flag else -0.1
                self.cmd_vel.publish(move_cmd)

            elif self.location['ball']['dist']<120:
                move_cmd = Twist()
                move_cmd.linear.x = -1
                move_cmd.angular.z = 0.1 if self.turn_flag else -0.1
                self.cmd_vel.publish(move_cmd)
